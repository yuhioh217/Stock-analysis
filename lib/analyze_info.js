...
module.exports = {
	table1 : async () => {
		...
		var table1_data = await mongodb.get({TimeStamp:date}, "Table1");

		while(table1_data.length == 0){
			date = date_check(date);
			table1_data = await mongodb.get({TimeStamp:date},"Table1");
		}

		var table1_sort = table1_data.sort(sortByProperty('Rank'));
		return table1_sort;
	},
	...
	table5 : async () => {
		...
		while(table5_data.length == 0){
			date = date_check(date);
			table5_data = await mongodb.get({TimeStamp:date},"Table5");
		}
